...
def get_visitor_centers_dict(filter):
    visitor_center_list = {}
    if filter == None:
        ...
        for x in filter_values: 
            visitor_centers_list.append(VisitorCenter.query.filter(VisitorCenter.states.like(x+"%")))
    visitor_centers = {}
...
def get_visitor_centers_list(filter=None):
